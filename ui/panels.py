...
class ControlPanel:
    ...
    def __init__(
        ...
        on_adversarial_action_change: Optional[Callable] = None,
    ) -> None:
        self.rect = rect
        ...
        self._on_adversarial_action_change = on_adversarial_action_change
    ...
    def _build_widgets(self) -> None:
        ...
        if group == 5:
            ...
            self._adversarial_start_label_y = self._map_y + 34
            self._adversarial_start_y = self._adversarial_start_label_y + 18
            self._mini_rect = pygame.Rect(x, self._adversarial_start_y + 42, w, 98)
            self._adversarial_y = self._mini_rect.bottom + 20
            self._controls_y = self._adversarial_y + 52
        else:
        ...
        self.btn_step = None
        self.btn_reset = None
        self.btn_adversarial_reset = None
        self._speed_buttons = []
        self.btn_compare = None
        self.adversarial_start_buttons: list[Button] = []
        if group != 5:
        ...
        self.adversarial_action_buttons: list[Button] = []
        self.adversarial_target_dropdown = None
        if group == 5:
            start_options = self._adversarial_start_options()
            selected_start = self.app_state.selected_start_node or (start_options[0] if start_options else "")
            gap = 8
            btn_w = (w - gap) // 2
            for idx, node_id in enumerate(start_options[:2]):
                self.adversarial_start_buttons.append(
                    Button(
                        pygame.Rect(x + idx * (btn_w + gap), self._adversarial_start_y, btn_w, 30),
                        node_id,
                        lambda n=node_id: self._set_adversarial_start_node(n),
                        color=(16, 94, 200) if node_id == selected_start else (7, 17, 31),
                    )
                )
            self.btn_adversarial_reset = Button(
                pygame.Rect(x, self._adversarial_y + 28, w, 28),
                "Reset ván đối kháng",
                self._on_reset,
                color=(198, 51, 50),
            )
        else:
            self._adversarial_y = None
            self.adversarial_action_buttons = []
            self.adversarial_start_buttons = []
    ...
    def _adversarial_start_options(self) -> list[str]:
        map_data = self.app_state.map_data
        if not map_data:
            return ["PC1", "PC2"]
        preferred = ["PC1", "PC2"]
        available = [node_id for node_id in preferred if map_data.graph.has_node(node_id)]
        if available:
            return available
        return [node.id for node in map_data.graph.get_all_nodes() if node.kind == "pc"][:2]
    ...
    def _set_adversarial_action(self, action: str) -> None:
        self.app_state.adversarial_hacker_action = action
        if self._on_adversarial_action_change:
            self._on_adversarial_action_change(action)
        self._build_widgets()

    def _on_adversarial_target_change(self, idx: int, node_id: str) -> None:
        self.app_state.adversarial_target_node = node_id
        self.app_state.selected_node = node_id
        self._build_widgets()

    def _set_adversarial_start_node(self, node_id: str) -> None:
        self.app_state.selected_start_node = node_id
        if self._on_start_node_change:
            self._on_start_node_change(0, node_id)
        self._build_widgets()

    def handle_event(self, event: pygame.event.Event) -> bool:
        consumed = False
        dropdowns = [self.algo_dropdown]
        if self.map_dropdown is not None:
            dropdowns.append(self.map_dropdown)
        if getattr(self, "adversarial_target_dropdown", None):
            dropdowns.append(self.adversarial_target_dropdown)
        ...
        for button in self.adversarial_action_buttons:
            consumed |= button.handle_event(event)
        for button in self.adversarial_start_buttons:
            consumed |= button.handle_event(event)
        if self.btn_adversarial_reset:
            consumed |= self.btn_adversarial_reset.handle_event(event)
        if getattr(self, "adversarial_target_dropdown", None):
            consumed |= self.adversarial_target_dropdown.handle_event(event)
        return consumed
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        if self.app_state.selected_group_index != 5:
            for button in self._algo_buttons:
                button.draw(surface)
        ...
        if self.map_dropdown is not None:
            ...
            self._draw_divider(surface, self._map_y - 10)
            self._draw_section(surface, 2, "TÁC VỤ HACKER", self._map_y)
            start_label = get_font(11).render("Điểm xâm nhập", True, COLOR_TEXT_SECONDARY)
            surface.blit(start_label, (self.rect.x + 14, self._adversarial_start_label_y))
            for button in self.adversarial_start_buttons:
                button.draw(surface)
            self._draw_mini_map(surface)
        ...
        if self.btn_adversarial_reset:
            self._draw_divider(surface, self._adversarial_y - 10)
            self._draw_section(surface, 3, "VÁN ĐỐI KHÁNG", self._adversarial_y)
            self.btn_adversarial_reset.draw(surface)
        self.algo_dropdown.draw_menu(surface)
        if self.map_dropdown is not None:
            self.map_dropdown.draw_menu(surface)
        if getattr(self, "adversarial_target_dropdown", None):
            self.adversarial_target_dropdown.draw_menu(surface)
